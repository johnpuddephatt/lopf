<div>
  <div class="container max-w-5xl mb-24 mt-36">
    ...
    isset(wp_get_attachment_metadata(get_post_thumbnail_id($post &&
    isset($post->ID) ? $post->ID : ''))['sizes']['square']))
    <div
      class="transform transform-gpu ml-auto  overlay w-3/4 md:w-80 @if(isset($parent)) md:-mb-24 @endif"
    >
      {!! get_the_post_thumbnail(isset($post->ID) ? $post->ID : '', 'square-s',
      ['class' => "object-cover object-center clip-teardrop w-full w-80 h-80
      md:w-full"]) !!}
    </div>
    @endif
    ...
    <div class="">
      ...
      <p
        class="max-w-xl mt-8 text-xl font-bold leading-tight tracking-tight md:text-2xl"
      >
        {!! $post->post_excerpt !!}
      </p>
      @endif @if(get_the_author_meta('ID') != 1)
      <div class="flex items-center py-2 mt-8">
        {!! get_avatar(get_the_author_meta('ID'), 32, null, 'Profile image for '
        . get_the_author(), [ "class" => "object-cover w-10 h-10 mr-2
        rounded-full" ]) !!}
