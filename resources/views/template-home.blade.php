...
>
  <div class="relative pr-8 bg-sky">
    ...
    >
      <div class="max-w-4xl pb-12 md:flex-1 md:pb-0">
        ...
        <p
          class="mt-4 antialiased font-semibold lg:text-xl md:mt-8 text-max-w-xl"
        >
          {!! $hero_subtitle !!}
        </p>
      </div>
      ...
      <div
        class="relative flex-none w-2/3 mb-6 ml-auto -mr-8 md:w-1/2 lg:my-0 md:pl-40 "
      >
        <div class="transform transform-cpu overlay">
          {!! $hero_image !!}
        </div>
      </div>
    </div>
  ...
  @endif @if($groups_enabled)
  <div
    class="container flex flex-col items-center space-y-8 lg:space-x-16 lg:space-y-0 lg:flex-row max-w-7xl"
  >
    <div class="flex-none w-full text-center md:py-12 md:w-2/5 ">
      <img alt="Map icon" src="@asset('/images/map.svg')" />
    </div>
    <div class="flex flex-col items-start space-y-6">
      <h2 class="font-serif text-4xl 2xl:text-5xl 2xl:leading-none text-blue">
        {!! $groups_title !!}
      </h2>
  ...
  @endif @if($blocks_enabled)

  <div class="container grid max-w-6xl grid-cols-1 gap-24 lg:grid-cols-2">
    @foreach($blocks as $block)
    <div class="flex flex-col space-y-8">
      <a href="{{ get_permalink($block->ID) }}"
        >{!! get_the_post_thumbnail($block->ID, 'twothirds', ['class' =>
        "rounded rounded-tr-big"]) !!}</a
      >
      <h3 class="text-3xl font-bold leading-tight text-purple">
        <a href="{{ get_permalink($block->ID) }}"
          >{!! get_the_title($block->ID) !!}</a
        >
      </h3>
      <div>
  ...
  @endif @if($research_enabled)
  <div
    class="container flex flex-col-reverse items-center max-w-5xl space-y-8 lg:space-x-8 lg:space-y-0 lg:flex-row"
  >
    <div class="flex flex-col items-start flex-1 space-y-6 ">
      <h2 class="font-serif text-4xl 2xl:text-5xl text-blue">
        {!! $research_title !!}
      </h2>
      <p>{!! $research_description !!}</p>
      <a
        href="{{ get_permalink(get_option('page_for_resources')) }}"
        class="px-8 py-2 text-lg font-bold border-2 rounded-2xl border-blue text-blue"
      >
  ...
  <div class="container relative pt-12 lg:pr-8">
    ...
    >
      ...
      >
        <h2 class="font-serif text-2xl antialiased text-white lg:text-4xl">
          {!! $join_pretitle !!}
        </h2>
        <p
          class="max-w-xl text-3xl antialiased font-bold leading-none lg:text-6xl lg:leading-none text-sky"
        >
          {!! $join_title !!}
        </p>
        <a
          href="/get-involved/sign-up-as-a-member-organisation/"
  ...
  <div
    class="container flex justify-center w-full space-y-8 bg-signup lg:space-x-8 lg:space-y-0 lg:flex-row"
  >
    <div
      class="flex flex-col items-center flex-1 max-w-2xl py-16 space-y-16 text-center"
    >
      <h2
        class="font-serif text-4xl leading-tight 2xl:leading-tight 2xl:text-5xl text-blue"
      >
        {!! nl2br($signup_title) !!}
      </h2>
      <form
      ...
      >
        <input
          ...
          class="flex-1 w-full px-8 py-2 text-lg border-2 rounded-2xl border-blue"
          type="text"
        />
        <input
          type="submit"
          value="{{ $signup_buttontext }}"
          class="px-8 py-2 text-lg font-bold text-white lowercase bg-purple rounded-2xl"
        />
      </form>
      <p class="text-sm">
        By submitting this form you acknowledge that the information you provide
        will be transferred to our marketing platform (Mailchimp) for processing
